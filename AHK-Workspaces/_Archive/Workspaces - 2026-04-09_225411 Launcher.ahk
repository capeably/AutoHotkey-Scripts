...
global TV_CTRL := "SysTreeView321"
global LV_CTRL := "SysListView321"
...
capture(){
	static node
	Gui,1:Default
	Gui,1:TreeView,%TV_CTRL%
	node:=settings.ssn("//*[@tv='" TV_GetSelection() "']"),node:=XPathNode(node,"ancestor-or-self::workspace")
	if !node
...
delete(){
	...
	if(Focus=TV_CTRL){
		...
		if(node.nodename="workspaces")
			return ShowMessage("Can not delete the main workspaces")
...
GuiControl,-Redraw,%TV_CTRL%
while,cc:=contract.item[A_Index-1]
	if(cc.nodename!="workspaces")
		TV_Modify(XPathNode(cc,"@tv").text,"-Expand")
GuiControl,+Redraw,%TV_CTRL%
return
toggle:
Gui,1:TreeView,%TV_CTRL%
...
if (tv.nodename!="Workspace"){
	ShowMessage("Select a Workspace to add windows to")
	return
}
Gui,3:Destroy
Gui,3:Default
Gui,Add,ListView,w400 h300,Window
Gui,Add,Button,gaddto Default,% "Add Selected to " XPathNode(tv,"@title").text
win:=settings.sn("//windows/*")
while,ww:=win.item[A_Index-1],ea:=xml.ea(ww)
	LV_Add("",ea.title)
Gui,Show,,% "Add To " XPathNode(tv,"@title").text
return
addto:
Gui,3:Default
Gui,3:ListView,%LV_CTRL%
next:=0
while,next:=LV_GetNext(next),LV_GetText(name,next)
	if !XPathNode(tv,"window[@title='" name "']")
...
PopulateGroups(save:=0){
	...
	for a,b in {workspaces:"Workspaces"}{
		...
		while,ii:=info.item[A_Index-1],ea:=xml.ea(ii){
			...
			ii.SetAttribute("tv",TV_Add(title value,XPathNode(ii.ParentNode,"@tv").text))
			if(ea.hotkey){
				Hotkey,IfWinActive
	...
	while,ss:=set.item[A_Index-1],ea:=xml.ea(ss){
		...
		tv:=TV_Add(ea.name hotkey,XPathNode(ss.ParentNode,"@tv").text),ss.SetAttribute("tv",tv)
		if(hotkey){
			Hotkey,IfWinActive
			Hotkey,% ea.Hotkey,hotkey,On
			lastkeys[ea.hotkey]:=1
	...
	select:=settings.sn("//*[@select]"),tv:=XPathNode(Select.item[0],"@tv").text,expand:=settings.sn("//*[@expand]"),VisFirst:=settings.ssn("//*[@VisFirst]/@tv").text
	while,ss:=select.item[A_Index-1]
		ss.RemoveAttribute("select")
...
AddHotkey(){
	...
	if !(InStr(workspace.nodename,"workspace")||workspace.nodename="setting")
		return ShowMessage("Please select a workspace to add a hotkey to")
	Gui,2:Destroy
	Gui,2:Default
	Gui,Add,Hotkey,w200 vhotkey hwndhotkeyhwnd,% XPathNode(workspace,"@hotkey").text
...
enter(){
	...
	if(focus=TV_CTRL){
		...
		if InStr(current.nodename,"workspace")
			...
			InputBox,newinfo,New Title,Enter a new title,,,,,,,,% XPathNode(current,"@title").text
...
update(){
	...
	if FileExist("gui.ahk")
		return
	URLDownloadToFile,https://files.maestrith.com/Workspace/Workspace.ahk,temp.ahk
...
ShowLauncher(){
	global LauncherHwnd,LauncherEditHwnd,LauncherListHwnd,LauncherItems,LauncherSel,LauncherQuery
	Gui,9:Destroy
	Gui,9:+AlwaysOnTop -Caption +Border +ToolWindow +HwndLauncherHwnd
	Gui,9:Margin,0,0
	Gui,9:Color,F5F5F5
	Gui,9:Font,s9 cAAAAAA,Segoe UI
	Gui,9:Add,Text,x0 y0 w364 h26 Center BackgroundDDDDDD hwndLauncherDragBarHwnd,Workspace Launcher
	Gui,9:Margin,12,12
...
LauncherCleanup(){
	...
	Hotkey,IfWinActive,ahk_id %LauncherHwnd%
	Hotkey,Enter,LauncherGo,Off
	Hotkey,Escape,LauncherClose,Off
	Hotkey,Down,LauncherNextItem,Off
	Hotkey,Up,LauncherPrevItem,Off
	Hotkey,IfWinActive
}
